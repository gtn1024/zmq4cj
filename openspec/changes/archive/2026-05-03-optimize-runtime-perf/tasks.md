...
- [x] 1.2 重写 `src/zmq_common.cj` 中的 `copyArrayToCPointer`，使用 `memcpy` + `acquireArrayRawData` 替代逐字节拷贝
- [x] 1.3 重写 `src/zmq_common.cj` 中的 `copyCPointerToArray`，使用 `memcpy` + `acquireArrayRawData` 替代逐字节拷贝
